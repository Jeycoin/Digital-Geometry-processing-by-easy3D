...
namespace easy3d {
	...
	void SurfaceMeshQEM::initialize(int cur_id, float aspect_ratio , float edge_length, unsigned int max_valence, float normal_deviation ) {
	...
	void SurfaceMeshQEM::enqueue_vertex(SurfaceMesh::Vertex v) {  //对顶点v0在队列中的值进行排序整理
		...
		//std::cout << "\n点" << v << "的最小边:" << min_h << "。 最小e:" << min_prio << std::endl;
	...
	float SurfaceMeshQEM::priority(const CollapseData &cd) {
		...
		//return abs(Q(newv));
		return Q(vpoint_[cd.v1]);     //此时为带入v1的边v0v1能量cost=Qei,j
	}

	vec3 SurfaceMeshQEM::compute_new_v(const CollapseData &cd){
	
		Quadric Q = vquadric_[cd.v0];
		Q += vquadric_[cd.v1];        //此时Q为边v0v1的二次误差cost=Qeij
		...
		{
			if (Q(vpoint_[cd.v0]) < Q(vpoint_[cd.v1])) 
			{
				if (Q(vpoint_[cd.v0]) < Q((vpoint_[cd.v0] + vpoint_[cd.v1]) / 2)) return new_v = vpoint_[cd.v0];
				else return new_v = (vpoint_[cd.v0] + vpoint_[cd.v1]) / 2;
			}
			else
			{
				if (Q(vpoint_[cd.v1]) < Q((vpoint_[cd.v0] + vpoint_[cd.v1]) / 2)) return new_v = vpoint_[cd.v1];
				else return new_v = (vpoint_[cd.v0] + vpoint_[cd.v1]) / 2;
			}
		}
		return new_v;
	...
	void SurfaceMeshQEM::collapse_qem_LML(SurfaceMesh::Halfedge h)
	{
